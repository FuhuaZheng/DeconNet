function beach_ball(fm, centerX, centerY, radius ,diam, ta, color)
...
X = cat(2,X1,Xs1,X2,Xs2)*R;
Y = cat(2,Y1,Ys1,Y2,Ys2)*R;

% phid = 0:.01:2*pi;
% [xx_b,yy_b] = pol2cart(phid,1);
% xx_b = xx_b*radius_r + CX;
% yy_b = ampy*yy_b*radius_r + CY;
...
if D > 0
   ...
   if ta == 0
      ...
      %line(xx_b,yy_b,'color','k','linewidth',0.2);
   else
      fillm(yy,xx,'w')
      fillm(X,Y,color)
      linem(yy,xx,'color','k','linewidth',0.2);
      %linem(yy_b,xx_b,'color','k','linewidth',0.2);
   end
else
   if ta == 0
      fill(X,Y,color)
