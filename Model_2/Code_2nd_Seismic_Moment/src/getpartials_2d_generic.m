function  [G,az,toa] = getpartials_2d_generic(mlats,mlons,melevs,late,lone,depe,Vp,Vs,topl,phas,strike,dip)
%
...
for i=1:nmeas
    ...
    [~,toa(i)]=traveltime(depth,ddum,[V topl]);
    ...
    rsv(i,3)=rv;
    rsv(i,1)=rh*cos(az(i)*pi/180);  %x positive north
    rsv(i,2)=rh*sin(az(i)*pi/180);  %y positive east
...
%     % now rotate to [Along strike (x'), fault normal(y'), DownDip(z')]	
    zz = strike;
    zz=zz*pi/180;
...
%      0, -sin(zz), cos(zz);];	
    ...
    partials(i,5)=2*s(1)*s(3); %Along-Strike --  DownDip
